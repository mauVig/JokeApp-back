          ...
          cargarAnecdotas()
            .then((y) => {
              cargarUsers()
                ...
                .catch((err) => {
                  console.log('Hubo un error en User: ' + err);
                });
            })
            .catch((err) => {
              console.log('Hubo un error en Anecdotas: ' + err);
            });
        })
        .catch((err) => {
          console.log('Hubo un error en Cuentos: ' + err);
        });
    })
    .catch((err) => {
      console.log('Hubo un error en Chistes: ' + err);
    });
});
...
async function cargarUsers() {
  ...
  for (const iterator of usuarios) {
    ...
    const pass = await bcrypt.hash(iterator.password, salt);
...
async function cargarAnecdotas() {
  const coll = 'anecdotas';
  for (const iterator of anecdotas) {
    await insert(
      {
        ...iterator,
        delete: false,
...
async function insert(obj, coll) {
  return conection(async function (db) {
    return await db.collection(coll).insertOne(obj);
  });
}
...
const chistes = [
  {
    text: '—¿Cuál es el colmo de un libro?',
    punchline: '-Que en otoño se caigan sus hojas.',
    positive: [],
    negative: [],
  },
  {
    text: '—Hoy he soñado que ganaba 200 millones como mi padre',
    punchline: '—¿Tu padre gana 200 millones? —No, él también lo sueña',
    positive: [],
    negative: [],
  },
  {
    text: '—¿Por qué lleva tanto papel higiénico?',
    punchline:
  ...
  {
    text: '—¿Qué le dice una pared a otra pared?',
    punchline: '—Nos vemos en la esquina.',
    positive: [],
    negative: [],
  },
  {
    text: '—¡Dile a tu hijo que deje de imitarme!',
    punchline: '—Jaimito, deja de hacer el imbécil',
    positive: [],
    negative: [],
  },
];
...
const Cuentos = [
  {
    author: 'Jorge Bucay',
    titulo: 'Dos números menos',
    text: `Un hombre entra en una zapatería, y un amable vendedor se le acerca:
...
- Me estoy matando con estos zapatos. Sufro terriblemente, es cierto... Pero, dentro de unas horas, cuando llegue a mi casa y me los quite, ¿imaginas el placer que sentiré? ¡Qué placer, tío! ¡Qué placer! `,
  ...
  },
  {
    ...
    text: `      Su luna de miel fue un largo escalofrío. Rubia, angelical y tímida, el carácter duro de su marido heló sus soñadas niñerías de novia. Lo quería mucho, sin embargo, a veces con un ligero estremecimiento cuando volviendo de noche juntos por la calle, echaba una furtiva mirada a la alta estatura de Jordán, mudo desde hacía una hora. Él, por su parte, la amaba profundamente, sin darlo a conocer.
...
Corrió hacia arriba y yo me quedé como un bobo, mirándola alejarse.`,
  ...
  },
  {
    ...
    titulo: 'Parábola del trueque',
    text: `Al grito de «¡Cambio esposas viejas por nuevas!» el mercader recorrió las calles del pueblo arrastrando su convoy de pintados carromatos.
...
const anecdotas = [
  {
    text: ` Una vez estaba hablando por whatsapp con mi crush y me dijo: tengo novia. Entonces pues yo casi me muero, quería saber quién era, me dice: se llama Isela, Isela Creyo. Yo muy enojada le pregunté a mis amigas si conocían a alguien llamada así y rieron de mí... luego entendí por qué. `,
    positive: [],
    negative: [],
  },
  {
    text: ` Un día con mis amigos pensábamos que el profe iba a faltar. Mis compañeros y yo nos pusimos a bailar sobre las mesas. De repente llega el profe y pone una cara de asombro. Pensamos que nos iba a regañar, pero no, él también se subió a su escritorio y bailó... aún tengo el video jaja `,
    positive: [],
    negative: [],
  },
  {
    text: ` Estaba un día en mi casa alistándome para salir a la escuela, pero ya se me hacía tarde, así que me cambie de ropa como sea. Tomé cualquier cosa y me la puse. Al llegar a clases me di cuenta, que mis medias eran diferentes. Cuando me decían ¿que paso? yo simplemente les respondía "es la nueva moda" ¡que bochorno! `,
    positive: [],
    negative: [],
  },
  {
    text: `Un día salí muy temprano de casa y me encontré con algo que me congeló la sangre: como a veinte metros de mí estaba acostado un perro de raza rottweiler, negro, pecho amarillo, cara de gángster (sólo le faltaban los lentes oscuros) era la mismísima encarnación canina de lucifer o la versión perruna de Hitler... Pero bueno, no fue el perro en sí lo que me asustó: de hecho, hay un impulso demoníaco en mi que me provoca gran placer ante la idea de ser perseguida por un perro.... tu sabes, la adrenalina.

Bueno, lo que más me asustó fue que cerca de él, estaban dos nenitas descalzas ¡Golpeando al Perro! Una le pateaba la panza y la otra le jalaba las orejas....¡Una desgracia total! O una hermosa escena surrealista a no ser por el peligro evidente que ahí había..... Cuando me detuve a observar aquello, el perro se levantó con violencia, iba a atacar. Aquel impulso de levantarse arrancó gritos de pánico a las bebitas, el perrote se disponía a descuartizar.....¡A mi! pues el perro dio una carrera hacia donde yo estaba... el pánico que sentí se convirtió en valor y logré montarme encima de un árbol de mango.
...
El monstruo puso sus patas luciferinas encima del árbol y me labraba, cuando supo que no me alcanzaría nunca inteligentemente se sentó a esperar que me bajara mientras me miraba con los ojos desorbitados y movía frenéticamente el tronquito donde una vez tuvo una cola. Las nenitas a quienes no les pasó nada corrieron hacia el árbol y con sus deditos me señalaban mientras se reían.... A mi solo me quedó el corazón a millón y decirles tiernamente: "Oye Mija" llama a tu mamá; quién afortunadamente no tardó en hacer su aparición y con cierta sonrisa grito: Piiiinky ven acá.....¿Pinki? ¿Quién llama a un perro rottweiler Pinki?`,
  ...
  {
    text: ` Un día mi hermana pequeña me dijo que jamás volvería a comer pescado, que no le gustaba. Entonces se me ocurrió engañarla y decirle que no era pescado, sino pollo italiano importado. Ella muy creída, empezó a comerse "el pollo" hasta que llegó mi mamá, entonces le dijo "¡Prueba este pollo!", hasta ahí llegó mi mentira.`,
    positive: [],
    negative: [],
  },
  {
    text: `Pasó el 24 de Diciembre del 2014; estaba esperando el 57 en Moreno para ir a lo de mi viejo (Martinez) y de repente una vieja en la cola (60 años aprox) me pregunta si quería viajar gratis, y antes que piensen que la anécdota es sobre algo sexual me explica que tiene un carnet de discapacidad, pero que el mismo requiere que viaje una persona que la cuide con ella. Me explica que no todos los colectiveros le piden que esté acompañada pero los del 57 la mayoría de veces sí, entonces ella le decía al chofer que yo era su nieto y podíamos viajar gratis ambos, ya que encima bajábamos en la misma parada. Cerraba perfecto, por ende digo que sí, más que nada para hacerle la gamba a la señora.

Lógicamente nos sentamos juntos y empezamos a charlar de las cosas que uno charla a los 23 años con una vieja de 60 en el 57 un 24 de diciembre. Me contó que tenía una rodilla medio al horno, que tenía una protésis de titanio (!) y luego que estaba yendo a lo de su hermana a pasar nochebuena y de paso llevaba sus herramientas de manicura para hacerle las uñas a todos allá, que era su vocación (ponele). El viaje transcurre mientras hablamos de lo más normal y cuando llegamos a la parada, bajamos.

Bien, yo estaba en el CBC de Medicina en ese entonces y estaba re cebado con la carrera, buscando info afuera de lo que te enseñaban y chusmeando cuanto pudiera sobre el tema. Hacía exactamente una semana yo había leído como hacer un torniquete ante una hemorragia, sin saber que iba a aplicar dicho conocimiento 7 días después. Nos bajamos del bondi y yo veo que la mina tenía un fierro que le salía de la gamba, del largo de un tenedor aprox. Le digo "¿qué tenés ahí?" y se me cruza por la cabeza que era la prótesis de la mina, que se había salido (esas pelotudeces que uno piensa cuando está pasando algo re wtf). La mina dice "uy" y toma el elemento metálico con la mano y lo extrae... Era un bisturí. What. The. Fuck. Tenía un bisturí enterrado en el muslo, y un bisturí grande. Se lo saca y empieza a salir sangre a chorros re epicamente y yo ante el grito de "QUEEE?" empiezo a intentar entender qué carajo estaba sucediendo. La mina dice "nooo, mirá lo que pasó... ". Yo intento buscar alguien que le pudiera haber clavado el bisturí para robarle o algo todavía queriendo entender qué mierda pasaba, y la mina me dice "es el bisturí que uso para las uñas" (wtf).

La mina tenía un bisturí en todo el set de herramientas de manicura. Cuando nos bajamos del bondi, se puso las bolsas encima y el bisturí sin capuchón traspasó la bolsa, calza, y pierna. Premio Darwin por llevar un bisturí sin capuchón en un transporte público.

Volviendo a la escena más extraña de mi vida, se me ocurre intentar hacerle el torniquete, se lo digo y saco la camisa que tenía en la mochila para ponerme esa noche, y me dice la vieja "no no que te la vas a manchar toda, usá esto" y me da un pañuelito que tenía en el cuello. Empecé a hacerlo re despesperado y pensando que no me iba a salir ni en pedo, y de orto salió. La pierna sangraba mucho y con el torniquete seguía sangrando pero muchísimo menos, lo cual alivió una bocha la situación. Justo había una remisería ahí con un único remís, al cual ya se estaba subiendo una mina con unos pibitos, pero le pedí que la llevaran al hospital y no hubo drama. Nunca más supe sobre la vieja pero no creo que haya pasado de unos puntos. Siempre me dieron ganas de encontrármela de vuelta y preguntarle en qué quedó el día. Muy loco todo.

TL;DR: le hice un torniquete a una vieja que conocía hacía una hora por tener un bisturí clavado en el muslo.`,
    positive: [],
    negative: [],
  },
];
...
const usuarios = [
  {
    nombre: 'Mauro',
    apellido: 'Vigliero',
    email: 'mauro.vigliero@yahoo.com',
    password: 'asdasd',
    materialEscritoId: [],
    favoritos: [],
    rol: 'admin',
